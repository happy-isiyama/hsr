...
class Arm():
    ...
    def instance_desk(self,object_name):
        ...
        print(listener)
        #listener.waitForTransform("/head_rgbd_sensor_link", topic_tf, rospy.Time(), rospy.Duration(4.0))
        #while not rospy.is_shutdown():
        #    try:
        #        now = rospy.Time.now()
        #        listener.waitForTransform("/head_rgbd_sensor_link", topic_tf, now, rospy.Duration(4.0))
        #        (trans,rot) = listener.lookupTransform("/odom", topic_tf, now)
        #    except:
        #        print("a")
        #        pass
    ...
    def instance_floor(self,object_name):
        topic_tf = object_name
        object_to_hand = geometry.pose(y=-0.2, ek=-1.57)
