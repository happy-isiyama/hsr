...
#床に落ちているオブジェクトを把持する。　手先は下に向けた状態で把持する。
    ...
    def place_object2(self,table_x, table_y, table_z, table_roll, table_pitch, table_yaw):
            # Set the reference frame for the end-effector
        ref_frame_id = 'odom'
    
        # Create the Pose object for the end-effector
        pose = geometry_msgs.msg.Pose()
        pose.position.x = table_x
        pose.position.y = table_y
        pose.position.z = table_z
        q = quaternion_from_euler(table_roll, table_pitch, table_yaw)
        pose.orientation = geometry_msgs.msg.Quaternion(*q)
    
        # Move the end-effector to the target pose
        try:
            whole_body.move_end_effector_pose(pose, ref_frame_id)
        except exceptions.PlanningError as e:
            rospy.logerr("Failed to plan a path: {}".format(e))
        except exceptions.ControlError as e:
            rospy.logerr("Failed to execute planned path: {}".format(e))
